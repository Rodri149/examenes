...
const existencia =inventory.filter((item) => item.quantity > 10);
...
//Cuantas cuantas marcas hay
const contar =marcas.reduce((cuenta) => cuenta + 1, 0);
console.log("Marcas: ",contar);

//const Ordenar las marcas desde la a ala z
const marcasOrdenadas =marcas.sort((a, b) => a.marca.localeCompare(b.marca));
console.log("Marcas ordenadas: ",marcasOrdenadas);


//Dar el pruducto con mayor existencia en el inventario
const  ProductoConmayorExistencia = inventory.reduce((max, actual) =>{
    if (actual.quantity > max.quantity) {
        return actual;
    }
    return max;
}, inventory[0]);
console.log(`El Producto con mayor existencia: ${ProductoConmayorExistencia.name}`);


//cual es el producto con mayor existensia en el inventario
const maxExistencia =inventory.reduce((max, item) => Math.max(max, item.quantity), 0);
console.log("El producto con mayor existencia es: ",maxExistencia);

//Cuantos Heroes hay por cada ciudad
const heroesPorCiudad =heroes.reduce((ciudad, heroe) => {
    if (!ciudad[heroe.city]) {
        ciudad[heroe.city] = 1;
    } else {
        ciudad[heroe.city]++;
    }
...
console.log("Heroes por ciudad: ",heroesPorCiudad);


//Cual es la ciudad que tiene mas heroes 
const maxheroes = heroes.reduce((max,ciudad) => {
    if (ciudad.city > max.city) {
        return ciudad;
    }
    return max;
}, heroes[0]);
...
//Promedio de edades
const promedioEdad =people.reduce((total, person) => total + person.edad, 0) / people.length;
console.log("El promedio de edades es: ",promedioEdad);